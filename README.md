...
### 3차원 BFS란?
- BFS는 너비우선탐색으로 그래프에서 단순 상하좌우 이동을 할 때 많이 활용한다.
- 3차원 BFS는 이러한 BFS 이동을 수행할 때, 특정 조건들이 추가된 경우 활용한다.
    - 예를 들어, 벽을 부숴야하는 경우가 있다면 `visited = [[0, 0, ...], ...]`에서 `visited = [[[0,0,...], [0,0,...], ...], ...]`으로 되며, 기존에는 "벽을 부수지 않았을 때의 이동 횟수"만 존재했다면 3차원 BFS는 "벽을 부수지 않았을 때의 이동 횟수", "벽을 한 번 부쉈을 때의 이동 횟수", "벽을 두 번 부쉈을 때의 이동 횟수" ... 로 확장된 형태이다.
    - 장기의 말(horse) 이동 방식, 점프 이동을 할 수 있고, 이에 대하여 **횟수 제한**이 있을 때.

### 작동 원리
- 작동 원리는 기본적인 BFS와 동일
- 단순히, "특이 케이스"를 "몇 번" 시행했는 지에 대한 visited에 값을 삽입해주는 차이만이 존재
